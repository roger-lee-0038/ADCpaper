...
As presented in Fig.~\ref{SAR}, a SAR sub-ADC is composed of two input buffers of the reference voltages, an array of capacitors, a comparator and a module of SAR logic. While  generating the upper 4-bit results, $V_{X}$ in the SAR sub-ADCs will be changed according to SAR logic. That means after 4 comparisons 
...
It is worth noting that we assign 14 steps for the 4 comparisons with SAR logic. Both of the first and second comparison takes 4 steps and the following two comparison takes 3 steps. It is because that the more rapidly $V_{X}$ can be changed, the more time of comparison may need to take. As for the last comparison with SS logic, taking 1 step is enouph because $V_{X}$ will exceed $V_{ref}$ gradually allowing the comparators' in-time response.
