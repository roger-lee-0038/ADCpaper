...
As we can see, most parts of the power consumption are taken up by the column-parallel comparators and the output buffer of the ramp generator, 
...
While the throughput is set similar to the SS ADCs, fewer steps is required in the SAR/SS ADCs, where 1 step counts for 2 clock periods. 
The SNDR is 24.25/57.87 dB and the ENOB is 3.74/9.32 bits, consistent with the design specifications. 
The power consumption is 256.1uW/column for high-precison conversion and 137.1uW/column for low-precison conversion, which means the energy of the SAR/SS ADCs can also be saved to nearly half.
According to the energy distribution analysis presented in Fig.~\ref{SARresults1}, most parts of the SAR/SS ADCs' power consumption are taken up by the column-parallel buffers of reference voltages in the sub-ADCs. And these parts of power consumption can be significantly reduced for low-precision conversion.
